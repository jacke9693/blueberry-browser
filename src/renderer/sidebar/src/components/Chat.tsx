...
import { ArrowUp, Plus, Settings, Wrench, Check, Loader2 } from "lucide-react";
...
const ToolActivityDisplay: React.FC<{ activities: ToolActivity[] }> = ({
  activities,
}) => {
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {activity.status === "calling" ? "Running..." : "Complete"}
              </span>
            </div>
            ...
            {activity.status === "complete" &&
              activity.result !== undefined && (
                <details className="mt-1">
                  ...
                  <pre className="mt-1 text-xs bg-background/50 p-2 rounded overflow-x-auto max-h-32 overflow-y-auto">
...
// Main Chat Component
export const Chat: React.FC<{ onOpenSettings: () => void }> = ({
  onOpenSettings,
}) => {
  const { messages, isLoading, toolActivity, sendMessage, clearChat } =
    useChat();
